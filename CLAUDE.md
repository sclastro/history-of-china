...
事件今譯改寫已完成 41 個（連同試點），附原文選段；進度及待辦見下文「今譯改寫進度」。
...
- 待辦：其餘 21 個事件（由商鞅之死至鄒忌諷諫）（`poe_rewrite.py --all` 自動略過已有 `original` 者）、成語 `translation` 欄、人物小傳夾註。

## 已知問題

- 「爾虞我詐」成語頁（`idioms/er-yu-wo-zha/`）說楚軍「只剩七天糧食」，未註出處；此事不見於《左傳》，
  出自《公羊傳》宣公十五年，宜補註出處。楚圍宋事件敘事已註明。
- 城濮之戰有兩段原文出自僖公二十七年，但 `sources` 只列二十八年，這兩段因此沒有 ctext 連結；
  按規則不自擬連結，待人手核實 `ctp:chun-qiu-zuo-zhuan/xi-gong-er-shi-qi-nian` 後補入 `sources`。
  同理，秦納重耳引《史記·晉世家》、三家分晉引〈周本紀〉〈晉世家〉的段落暫無連結。
